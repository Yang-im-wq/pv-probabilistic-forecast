...
|---|---|---|---|---|---|
| 点预测 LSTM | 2.285 | 1.301 | 0.0505 | — | — |
| 分位数回归 LSTM（q50） | 2.297 | 1.228 | 0.0508 | **0.8095** | **0.0831** |

> 目标为 50 MW 光伏站，RMSE ≈ 2.3 MW ≈ 容量的 4.6%。对"仅用本地气象、15 分钟超短期、无数值天气预报（NWP）"的设定，这是合理水平；加入 NWP 或云图后可进一步降低。

**指标怎么读**：
- **PICP = 区间覆盖率**：标称 90% 的区间实际覆盖 81.0%，**略低于标称**——说明区间略微"过自信"（偏窄）。这是真实光伏数据的正常现象（云团遮挡的不确定性远大于合成数据），可通过增加分位数（如 9 个）、或对区间做事后校准来改善。这个"略欠覆盖"本身就是一个诚实的、可说清的实验结果。
- **PINAW = 归一化区间宽度**：0.083 意味着平均区间宽度约为目标极差的 8.3%，区间仍较"紧"。
- 两者一起看：PICP 接近标称且 PINAW 尽量小 = 又准又紧；PICP 明显偏低 = 区间太窄不可全信；PICP 很高但 PINAW 很大 = 区间太宽、无信息量。
